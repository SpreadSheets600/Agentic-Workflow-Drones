...
class PreFlightSystemCheck:
    ...
    def run(self, state, drone, vision):
        ...
        for name, action_type, critical in self.CHECKS:
            ...
            observation = self._check(action_type, drone, vision) if allowed else ToolObservation(
                success=False, toolName=action_type.value, message=reason, data={}
            )
            result = PreFlightResult(name, observation.success, critical, observation.message)
            results.append(result)
            state.preflight_results.append({"name": name, "passed": result.passed, "critical": critical, "message": result.message})
            state.record_event("PRE_FLIGHT_CHECK", f"{name}: {'PASS' if result.passed else 'FAIL'}")

        state.mission_ready = all(item.passed for item in results if item.critical)
        state.readiness_reason = "All critical systems are available." if state.mission_ready else next(
    ...
    def _check(action_type, drone, vision):
        ...
        gps_check = lambda: ToolObservation(drone.gps_available, "check_gps", f"GPS / Positioning {'Available' if drone.gps_available else 'Unavailable'}.", {"available": drone.gps_available})
        checks = {
            ActionType.CHECK_GPS: gps_check,
            ActionType.CHECK_BATTERY: drone.check_battery,
            ActionType.CHECK_CAMERA: camera_check or (lambda: ToolObservation(True, "check_camera", "Camera Available.", {})),
