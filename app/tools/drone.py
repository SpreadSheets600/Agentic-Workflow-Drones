...
class MockDrone:
    def __init__(
        self, gps_available=True, navigation_available=True, geofence_available=True
    ):
        self.battery = 100.0
        self.location = "BASE"
    ...
    def _capability(self, name, available):
        return ToolObservation(
            available,
            name,
            f"{name.replace('_', ' ').title()} {'Available' if available else 'Unavailable'}.",
            {"available": available},
        )
    ...
    def check_battery(self):
        return ToolObservation(
            self.battery >= 30,
            "check_battery",
            f"Battery Level Is {self.battery:.0f}%.",
            {"battery": self.battery},
        )

    def check_navigation_controller(self):
        return self._capability(
            "check_navigation_controller", self.navigation_available
        )
